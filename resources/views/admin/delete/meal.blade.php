        ...
        <div class="content-wrapper">
            <div class="content-header row">
                <div class="content-header-left col-md-6 col-xs-12 mb-1">
                    <h2 class="content-header-title">Deleted Meal</h2>
                </div>
                <div class="content-header-right breadcrumbs-right breadcrumbs-top col-md-6 col-xs-12">
                    <div class="breadcrumb-wrapper col-xs-12">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">Meal
                            </li>
                        </ol>
                    </div>
                </div>
                                    ...
                                    <table class="table table-bordered mb-0" id="example">
                                        ...
                                        @foreach($meals as $key=>$meal)
                                            <tr data-route="{{url("meal/{$meal->id}")}}">
                                                {{--                                            <td>{{$key+1}}</td>--}}
                                                <td>{{$meal->menu->restaurant->name}}</td>
                                                <td>{{$meal->menu->name_sv}}</td>
                                                <td>{{$meal->name_sv}}</td>
                                                <td>{{$meal->description_sv}}</td>
                                                <td>{{$meal->price}}</td>
        ...
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel19">Recovery Meal</h4>
                </div>
                <div class="modal-body">
                    <h5>Are you sure you want to Recovery this Meal ?!</h5>
                </div>
...
@section('script')
    ...
    {{--/* for active sidebar*/--}}
    <script>
        var APP_URL = {!! json_encode(url('/deleted_meal')) !!}
